...
class MaskDINOEncoder(nn.Module):
    ...
    def forward_features(self, features, masks):
        """  forward两个结构: MSDeformAttnTransformerEncoderOnly + FPN
        ...
        srcsl = []
        posl = []
        if self.total_num_feature_levels > self.transformer_num_feature_levels:  # 5 > 4
            # proj 最小的feat
            smallest_feat = features[self.transformer_in_features[
                self.low_resolution_index]].float()  # 最小的feat
            _len_srcs = self.transformer_num_feature_levels  # 4
            for l in range(_len_srcs, self.total_num_feature_levels):
                if l == _len_srcs:
                    src = self.input_proj[l](smallest_feat)  # 对最高feat 进行 input_proj
                else:
        ...
        # 根据特征顺序决定是将srcs扩展到srcsl还是将srcsl扩展到srcs
        # 如果特征顺序为'low2high'，则将srcs扩展到srcsl；否则将srcsl扩展到srcs
        srcs.extend(
            srcsl) if self.feature_order == 'low2high' else srcsl.extend(srcs)
        # 根据特征顺序决定是将pos扩展到posl还是将posl扩展到pos
        # 如果特征顺序为'low2high'，则将pos扩展到posl；否则将posl扩展到pos
        pos.extend(posl) if self.feature_order == 'low2high' else posl.extend(
            pos)
        if self.feature_order != 'low2high':
            srcs = srcsl
            pos = posl
        # 使用MSDeformAttnTransformerEncoderOnly对srcs, masks, pos进行转换，返回转换后的特征y，以及空间形状和层级起始索引
        y, spatial_shapes, level_start_index = self.transformer(
            srcs, masks, pos)
        bs = y.shape[0]
        # 初始化一个列表，用于存储每个特征级别对应的分割大小
        split_size_or_sections = [None] * self.total_num_feature_levels
        
        # 遍历特征级别
        for i in range(self.total_num_feature_levels):
            # 对于除最后一级外的级别，计算当前级别与下一级别起始索引的差值作为分割大小
            if i < self.total_num_feature_levels - 1:
                split_size_or_sections[i] = level_start_index[
                    i + 1] - level_start_index[i]
            # 对于最后一级，计算从当前级别起始索引到数据末尾的长度作为分割大小
            else:
                split_size_or_sections[i] = y.shape[1] - level_start_index[i]
        
        # 根据计算得到的分割大小列表，将张量y按列分割成多个部分
        y = torch.split(y, split_size_or_sections, dim=1)

        out = []
        multi_scale_features = []
        num_cur_levels = 0
        # 遍历每个特征图，并根据其索引和形状进行转置和重塑
        for i, z in enumerate(y):
            # 将特征图z转置以改变通道和高度/宽度的顺序
            # 然后通过重塑操作将其形状变更为(batch size, channel数, 高度, 宽度)
            out.append(
